...
<script setup>
import {NCard, NTooltip, NCode, NIcon} from 'naive-ui';
...
const copy = () => {
   var range = document.createRange();
    range.selectNode(document.getElementById("wdCode"));
    window.getSelection().removeAllRanges(); // clear current selection
    window.getSelection().addRange(range); // to select text
    document.execCommand("copy");
    window.getSelection().removeAllRanges();// to desele

    tooltipText.value = 'Código copiado'
    setTimeout(() => {
      tooltipText.value = 'Copiar código'
    }, 2000);
}
</script>
<style lang="scss">
